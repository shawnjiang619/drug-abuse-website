...
},{}],"EXky":[function(require,module,exports) {
var define;
...
(function (root, factory) {
  ...
  var _version = "0.2.1"; // used to uniquely generate IDs and classes, ensuring no conflict between multiple pies on the same page

  var _uniqueIDCounter = 0; // this section includes all helper libs on the d3pie object. They're populated via grunt-template. Note: to keep
  ...
  var validate = {
    ...
    initialCheck: function initialCheck(pie) {
      var cssPrefix = pie.cssPrefix;
      var element = pie.element;
      var options = pie.options; // confirm d3 is available [check minimum version]
      ...
      pie.options.data.content = data; // labels.outer.hideWhenLessThanPercentage - 1-100
  ...
  var helpers = {
    ...
    addSVGSpace: function addSVGSpace(pie) {
      var element = pie.element;
      var canvasWidth = pie.options.size.canvasWidth;
      var canvasHeight = pie.options.size.canvasHeight;
      var backgroundColor = pie.options.misc.colors.background;
      var svg = d3.select(element).append("svg:svg").attr("width", canvasWidth).attr("height", canvasHeight);
    ...
    whenIdExists: function whenIdExists(id, callback) {
      var inc = 1;
    ...
    whenElementsExist: function whenElementsExist(els, callback) {
      var inc = 1;
      var giveupIterationCount = 1000;
      var interval = setInterval(function () {
        var allExist = true;
    ...
    processObj: function processObj(obj, is, value) {
      if (typeof is === 'string') {
    ...
    rectIntersect: function rectIntersect(r1, r2) {
      var returnVal = // r2.left > r1.right
      r2.x > r1.x + r1.w || // r2.right < r1.left
      r2.x + r2.w < r1.x || // r2.top < r1.bottom
      r2.y + r2.h < r1.y || // r2.bottom > r1.top
      r2.y > r1.y + r1.h;
      return !returnVal;
    },
     ...
     * @returns {string}
     */
    getColorShade: function getColorShade(hex, lum) {
      // validate hex string
     ...
     * @param data
     */
    initSegmentColors: function initSegmentColors(pie) {
      var data = pie.options.data.content;
      var colors = pie.options.misc.colors.segments; // TODO this needs a ton of error handling
    ...
    applySmallSegmentGrouping: function applySmallSegmentGrouping(data, smallSegmentGrouping) {
      var totalSize;
    ...
    // for debugging
    showPoint: function showPoint(svg, x, y) {
  ...
  var math = {
    ...
    computePieRadius: function computePieRadius(pie) {
      var size = pie.options.size;
      var canvasPadding = pie.options.misc.canvasPadding; // outer radius is either specified (e.g. through the generator), or omitted altogether
      ...
      var h = size.canvasHeight - canvasPadding.top - canvasPadding.bottom; // now factor in the footer, title & subtitle
      ...
      var innerRadius, percent; // if the user specified something, use that instead
      ...
      if (size.pieOuterRadius !== null) {
        if (/%/.test(size.pieOuterRadius)) {
          ...
          var smallestDimension = w < h ? w : h; // now factor in the label line size
    ...
    getTotalPieSize: function getTotalPieSize(data) {
      var totalSize = 0;
    ...
    sortPieData: function sortPieData(pie) {
      var data = pie.options.data.content;
    ...
    getPieTranslateCenter: function getPieTranslateCenter(pieCenter) {
     ...
     * @private
     */
    calculatePieCenter: function calculatePieCenter(pie) {
     ...
     * @returns {Array}
     */
    rotate: function rotate(x, y, xm, ym, a) {
     ...
     * @param a angle in degrees
     */
    translate: function translate(x, y, d, a) {
    ...
    pointIsInArc: function pointIsInArc(pt, ptData, d3Arc) {
  ...
  var labels = {
     ...
     * @param pie
     */
    add: function add(pie, section, sectionDisplayType) {
      var include = labels.getIncludes(sectionDisplayType);
      var settings = pie.options.labels; // group the label groups (label, percentage, value) into a single element for simpler positioning

      var outerLabel = pie.svg.insert("g", "." + pie.cssPrefix + "labels-" + section).attr("class", pie.cssPrefix + "labels-" + section);
      var labelGroup = outerLabel.selectAll("." + pie.cssPrefix + "labelGroup-" + section).data(pie.options.data.content).enter().append("g").attr("id", function (d, i) {
        return pie.cssPrefix + "labelGroup" + i + "-" + section;
      }).attr("data-index", function (d, i) {
        return i;
      }).attr("class", pie.cssPrefix + "labelGroup-" + section).style("opacity", 0);
      ...
      if (include.mainLabel) {
        labelGroup.append("text").attr("id", function (d, i) {
          return pie.cssPrefix + "segmentMainLabel" + i + "-" + section;
        }).attr("class", pie.cssPrefix + "segmentMainLabel-" + section).text(function (d, i) {
          var str = d.label; // if a custom formatter has been defined, pass it the raw label string - it can do whatever it wants with it.
          ...
          return str;
        }).style("font-size", settings.mainLabel.fontSize + "px").style("font-family", settings.mainLabel.font).style("fill", settings.mainLabel.color);
      ...
      if (include.percentage) {
        labelGroup.append("text").attr("id", function (d, i) {
        ...
        }).attr("class", pie.cssPrefix + "segmentPercentage-" + section).text(function (d, i) {
          ...
          return percentage;
        }).style("font-size", settings.percentage.fontSize + "px").style("font-family", settings.percentage.font).style("fill", settings.percentage.color);
      ...
      if (include.value) {
        ...
        }).attr("class", pie.cssPrefix + "segmentValue-" + section).text(function (d, i) {
          formatterContext.index = i;
          formatterContext.part = "value";
          formatterContext.value = d.value;
          formatterContext.label = d.value;
          return settings.formatter ? settings.formatter(formatterContext, d.value) : d.value;
        }).style("font-size", settings.value.fontSize + "px").style("font-family", settings.value.font).style("fill", settings.value.color);
      }
    },
     ...
     */
    positionLabelElements: function positionLabelElements(pie, section, sectionDisplayType) {
      labels["dimensions-" + section] = []; // get the latest widths, heights
    ...
    computeLabelLinePositions: function computeLabelLinePositions(pie) {
      pie.lineCoordGroups = [];
      d3.selectAll("." + pie.cssPrefix + "labelGroup-outer").each(function (d, i) {
        return labels.computeLinePosition(pie, i);
      });
    },
    computeLinePosition: function computeLinePosition(pie, i) {
      ...
      var midPoint = 4;
      var x2, y2, x3, y3; // this resolves an issue when the
    ...
    addLabelLines: function addLabelLines(pie) {
      var lineGroups = pie.svg.insert("g", "." + pie.cssPrefix + "pieChart") // meaning, BEFORE .pieChart
      .attr("class", pie.cssPrefix + "lineGroups").style("opacity", 0);
      var lineGroup = lineGroups.selectAll("." + pie.cssPrefix + "lineGroup").data(pie.lineCoordGroups).enter().append("g").attr("class", pie.cssPrefix + "lineGroup");
      var lineFunction = d3.line().curve(d3.curveBasis).x(function (d) {
        return d.x;
      }).y(function (d) {
        return d.y;
      });
      lineGroup.append("path").attr("d", lineFunction).attr("stroke", function (d, i) {
        return pie.options.labels.lines.color === "segment" ? pie.options.colors[i] : pie.options.labels.lines.color;
      }).attr("stroke-width", 1).attr("fill", "none").style("opacity", function (d, i) {
        var percentage = pie.options.labels.outer.hideWhenLessThanPercentage;
        var isHidden = percentage !== null && d.percentage < percentage || pie.options.data.content[i].label === "";
        return isHidden ? 0 : 1;
      });
    },
    positionLabelGroups: function positionLabelGroups(pie, section) {
      if (pie.options.labels[section].format === "none") {
        return;
      }

      d3.selectAll("." + pie.cssPrefix + "labelGroup-" + section).style("opacity", 0).attr("transform", function (d, i) {
        var x, y;
        ...
        if (section === "outer") {
          ...
          var pieCenterCopy = extend(true, {}, pie.pieCenter); // now recompute the "center" based on the current _innerRadius
    ...
    fadeInLabelsAndLines: function fadeInLabelsAndLines(pie) {
      ...
      setTimeout(function () {
        ...
        d3.selectAll("." + pie.cssPrefix + "labelGroup-outer").transition().duration(labelFadeInTime).style("opacity", function (d, i) {
          var percentage = pie.options.labels.outer.hideWhenLessThanPercentage;
          return percentage !== null && d.percentage < percentage ? 0 : 1;
        });
        d3.selectAll("." + pie.cssPrefix + "labelGroup-inner").transition().duration(labelFadeInTime).style("opacity", function (d, i) {
          var percentage = pie.options.labels.inner.hideWhenLessThanPercentage;
          return percentage !== null && d.percentage < percentage ? 0 : 1;
        });
        d3.selectAll("g." + pie.cssPrefix + "lineGroups").transition().duration(labelFadeInTime).style("opacity", 1); // once everything's done loading, trigger the onload callback if defined
    ...
    getIncludes: function getIncludes(val) {
     ...
     */
    computeOuterLabelCoords: function computeOuterLabelCoords(pie) {
     ...
     */
    resolveOuterLabelCollisions: function resolveOuterLabelCollisions(pie) {
      if (pie.options.labels.outer.format === "none") {
        return;
      }
    ...
    checkConflict: function checkConflict(pie, currIndex, direction, size) {
      var i, curr;
      ...
      var nextIndex = direction === "clockwise" ? currIndex + 1 : currIndex - 1; // this is the current label group being looked at. We KNOW it's positioned properly (the first item
      // is always correct)

      var currLabelGroup = pie.outerLabelGroupData[currIndex]; // this one we don't know about. That's the one we're going to look at and move if necessary
      ...
      if (direction === "clockwise") {
        ...
        for (; i <= currIndex; i++) {
          curr = pie.outerLabelGroupData[i]; // if there's a conflict with this label group, shift the label to be AFTER the last known
    ...
    isLabelHidden: function isLabelHidden(pie, index) {
      var percentage = pie.options.labels.outer.hideWhenLessThanPercentage;
      return percentage !== null && d.percentage < percentage || pie.options.data.content[index].label === "";
    },
    // does a little math to shift a label into a new position based on the last properly placed one
    adjustLabelPos: function adjustLabelPos(pie, nextIndex, lastCorrectlyPositionedLabel, info) {
     ...
     */
    getIdealOuterLabelPositions: function getIdealOuterLabelPositions(pie, i) {
      ...
      var newCoords = math.rotate(originalX, originalY, pie.pieCenter.x, pie.pieCenter.y, angle); // if the label is on the left half of the pie, adjust the values
  ...
  var segments = {
     ...
     * @private
     */
    create: function create(pie) {
      var pieCenter = pie.pieCenter;
      var colors = pie.options.colors;
      var loadEffects = pie.options.effects.load;
      var segmentStroke = pie.options.misc.colors.segmentStroke; // we insert the pie chart BEFORE the title, to ensure the title overlaps the pie

      var pieChartElement = pie.svg.insert("g", "#" + pie.cssPrefix + "title").attr("transform", function () {
        return math.getPieTranslateCenter(pieCenter);
      }).attr("class", pie.cssPrefix + "pieChart");
      var arc = d3.arc().innerRadius(pie.innerRadius).outerRadius(pie.outerRadius).startAngle(0).endAngle(function (d) {
        return d.value / pie.totalSize * 2 * Math.PI;
      });
      var g = pieChartElement.selectAll("." + pie.cssPrefix + "arc").data(pie.options.data.content).enter().append("g").attr("class", pie.cssPrefix + "arc"); // if we're not fading in the pie, just set the load speed to 0
      ...
      g.append("path").attr("id", function (d, i) {
        ...
        return color;
      }).style("stroke", segmentStroke).style("stroke-width", 1).transition().ease(d3.easeCubicInOut).duration(loadSpeed).attr("data-index", function (d, i) {
        return i;
    ...
    addGradients: function addGradients(pie) {
      var grads = pie.svg.append("defs").selectAll("radialGradient").data(pie.options.data.content).enter().append("radialGradient").attr("gradientUnits", "userSpaceOnUse").attr("cx", 0).attr("cy", 0).attr("r", "120%").attr("id", function (d, i) {
    ...
    addSegmentEventHandlers: function addSegmentEventHandlers(pie) {
      var arc = d3.selectAll("." + pie.cssPrefix + "arc,." + pie.cssPrefix + "labelGroup-inner,." + pie.cssPrefix + "labelGroup-outer");
      arc.on("click", function () {
        var currentEl = d3.select(this);
        var segment; // mouseover works on both the segments AND the segment labels, hence the following
    ...
    onSegmentEvent: function onSegmentEvent(pie, func, segment, isExpanded) {
    ...
    openSegment: function openSegment(pie, segment) {
      ...
      d3.select(segment).transition().ease(segments.effectMap[pie.options.effects.pullOutSegmentOnClick.effect]).duration(pie.options.effects.pullOutSegmentOnClick.speed).attr("transform", function (d, i) {
    ...
    maybeCloseOpenSegment: function maybeCloseOpenSegment(pie) {
      if (d3.selectAll("." + pie.cssPrefix + "expanded").size() > 0) {
        segments.closeSegment(pie, d3.select("." + pie.cssPrefix + "expanded").node());
      }
    },
    closeSegment: function closeSegment(pie, segment) {
      d3.select(segment).transition().duration(400).attr("transform", "translate(0,0)").on("end", function (d, i) {
        d3.select(segment).attr("class", "");
        pie.currentlyOpenSegment = null;
      });
    },
    getCentroid: function getCentroid(el) {
      var bbox = el.getBBox();
      return {
     ...
     */
    getSegmentAngle: function getSegmentAngle(index, data, totalSize, opts) {
      var options = extend({
      ...
      if (options.compounded) {
        fullValue = 0; // get all values up to and including the specified index
      ...
      var angle = fullValue / totalSize * 360; // lastly, if we want the midpoint, factor that sucker in
  ...
  var text = {
    ...
    addTitle: function addTitle(pie) {
      var title = pie.svg.selectAll("." + pie.cssPrefix + "title").data([pie.options.header.title]).enter().append("text").text(function (d) {
        return d.text;
      }).attr("id", pie.cssPrefix + "title").attr("class", pie.cssPrefix + "title").attr("x", text.offscreenCoord).attr("y", text.offscreenCoord).attr("text-anchor", function () {
        var location;
    ...
    positionTitle: function positionTitle(pie) {
      ...
      if (headerLocation === "pie-center") {
        y = pie.pieCenter.y; // still not fully correct
      ...
      pie.svg.select("#" + pie.cssPrefix + "title").attr("x", x).attr("y", y);
    },
    addSubtitle: function addSubtitle(pie) {
      var headerLocation = pie.options.header.location;
      pie.svg.selectAll("." + pie.cssPrefix + "subtitle").data([pie.options.header.subtitle]).enter().append("text").text(function (d) {
        return d.text;
      }).attr("x", text.offscreenCoord).attr("y", text.offscreenCoord).attr("id", pie.cssPrefix + "subtitle").attr("class", pie.cssPrefix + "subtitle").attr("text-anchor", function () {
        var location;
    ...
    positionSubtitle: function positionSubtitle(pie) {
      var canvasPadding = pie.options.misc.canvasPadding;
      var canvasWidth = pie.options.size.canvasWidth;
      var x;
      ...
      pie.svg.select("#" + pie.cssPrefix + "subtitle").attr("x", x).attr("y", y);
    },
    addFooter: function addFooter(pie) {
      pie.svg.selectAll("." + pie.cssPrefix + "footer").data([pie.options.footer]).enter().append("text").text(function (d) {
        return d.text;
      }).attr("x", text.offscreenCoord).attr("y", text.offscreenCoord).attr("id", pie.cssPrefix + "footer").attr("class", pie.cssPrefix + "footer").attr("text-anchor", function () {
        var location = "left";
    ...
    positionFooter: function positionFooter(pie) {
      ...
      pie.svg.select("#" + pie.cssPrefix + "footer").attr("x", x).attr("y", canvasHeight - canvasPadding.bottom);
    },
    getHeaderHeight: function getHeaderHeight(pie) {
      var h;
  ...
  var tt = {
    addTooltips: function addTooltips(pie) {
      ...
      tooltips.selectAll("." + pie.cssPrefix + "tooltip").data(pie.options.data.content).enter().append("g").attr("class", pie.cssPrefix + "tooltip").attr("id", function (d, i) {
        return pie.cssPrefix + "tooltip" + i;
      }).style("opacity", 0).append("rect").attr("rx", pie.options.tooltips.styles.borderRadius).attr("ry", pie.options.tooltips.styles.borderRadius).attr("x", -pie.options.tooltips.styles.padding).attr("opacity", pie.options.tooltips.styles.backgroundOpacity).style("fill", pie.options.tooltips.styles.backgroundColor);
      tooltips.selectAll("." + pie.cssPrefix + "tooltip").data(pie.options.data.content).append("text").attr("fill", function (d) {
      ...
      tooltips.selectAll("." + pie.cssPrefix + "tooltip rect").attr("width", function (d, i) {
  ...
  var d3pie = function d3pie(element, options) {
    ...
    this.options = opts; // if the user specified a custom CSS element prefix (ID, class), use it
    ...
    d3.select(this.element).attr(_scriptName, _version); // things that are done once
  ...
  var _setupData = function _setupData() {
    ...
    var dp = this.options.labels.percentage.decimalPlaces; // add in percentage data to content
  ...
  var _init = function _init() {
    // prep-work
    this.svg = helpers.addSVGSpace(this); // store info about the main text components as part of the d3pie object instance
    ...
    this.outerLabelGroupData = []; // add the key text components offscreen (title, subtitle, footer). We need to know their widths/heights for later computation
    ...
    text.addFooter(this); // the footer never moves. Put it in place now
    ...
    helpers.whenElementsExist(reqEls, function () {
      ...
      math.computePieRadius(self); // this value is used all over the place for placing things and calculating locations. We figure it out ONCE
      // and store it as part of the object

      math.calculatePieCenter(self); // position the title and subtitle

      text.positionTitle(self);
      text.positionSubtitle(self); // now create the pie chart segments, and gradients if the user desired
      ...
      labels.add(self, "outer", self.options.labels.outer.format); // position the label elements relatively within their individual group (label, percentage, value)

      labels.positionLabelElements(self, "inner", self.options.labels.inner.format);
      labels.positionLabelElements(self, "outer", self.options.labels.outer.format);
      labels.computeOuterLabelCoords(self); // this is (and should be) dumb. It just places the outer groups at their calculated, collision-free positions

      labels.positionLabelGroups(self, "outer"); // we use the label line positions for many other calculations, so ALWAYS compute them

      labels.computeLabelLinePositions(self); // only add them if they're actually enabled
      ...
      labels.fadeInLabelsAndLines(self); // add and position the tooltips
  ...
  var _getPercentage = function _getPercentage(value, total, decimalPlaces) {
...
},{}],"JHJD":[function(require,module,exports) {
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
function () {
  function MyPie() {
    ...
    return new d3pie("pie-chart", {
      ...
      "callbacks": {
        "onClickSegment": function onClickSegment(info) {
...
},{"./d3pie":"EXky"}],"dPF9":[function(require,module,exports) {
module.exports = {
  "example": [1, 2, 3, 4, 5],
  "data": [6, 7, 8, 9, 10]
};
...
var d3 = require('d3'); // const fullpage = require('fullpage.js')
...
var MyPie = require('./pie');

var pie = new MyPie(); // You can load JSON files directly via require.
...
// });
},{"d3":"UzF0","./my-class":"elfx","./pie":"JHJD","./example-data.json":"dPF9"}]},{},["Focm"], null)
//# sourceMappingURL=https://uw-cse442-wi20.github.io/FP-understanding-drug-abuse/src.5cf36dd6.js.map